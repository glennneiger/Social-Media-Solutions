...
function td_nm_get_notifications( $filters = array() ) {
	...
	$tvo_active = is_plugin_active( 'thrive-ovation/thrive-ovation.php' );
	...
	foreach ( $posts as $post ) {
		 ...
		 */
		if ( ! $tl_active && ! empty( $settings['tl'] ) && empty( $settings['tho'] ) && ! array_key_exists( 'tvo', $settings ) ) {
			continue;
		}
		 ...
		 */
		if ( ! $tho_active && empty( $settings['tl'] ) && empty( $settings['groups'] ) && empty( $settings['shortcodes'] ) && empty( $settings['thrive_boxes'] ) && ! array_key_exists( 'tvo', $settings ) ) {
			continue;
		}

		/**
		 * Thrive Ovation is not active and tvo key does not exist
		 */
		if ( ! $tvo_active && ! array_key_exists( 'tvo', $settings ) && empty( $settings['tl'] ) && empty( $settings['groups'] ) && empty( $settings['shortcodes'] ) && empty( $settings['thrive_boxes'] ) && empty( $settings['tho'] ) ) {
			continue;
		}
